...
export default class Section extends Component <SectionProps> {
    render() {
        if(this.props.loaded) {
            return <div className={(this.props.isFirst ? "bg-violet-700 p-4 cover abt" : this.props.noBg ? "p-4 cover2" : "bg-violet-900 p-4 cover2") + " " + this.props.className + " text-center p-20"}>
                <p>Loading dynamic content...</p>
            </div>;
        }

        return (
            <section key={this.props.title} className={ (this.props.isFirst ? " bg-violet-700 p-4 cover abt" : this.props.noBg ? "p-4 cover2" : "bg-violet-800 p-4 cover2") + " z-10 " + this.props.className}>
